...
module.exports = (cwd, config) => {
    ...
    const template = handlebars.compile(fs.readFileSync(path.resolve(templatePath, config.template, 'page.hbs'), { encoding: 'utf8' }));
    ...
    docs.forEach((pathName) => {
        ...
        if (typeof page.template === 'undefined') {
            builtHTML = template(templateInfo);
        } else if (page.template === 'none') {
            builtHTML = docData.result;
        } else {
            const userTemplate = handlebars.compile(fs.readFileSync(path.resolve(templatePath, config.template, `${page.template}.hbs`), { encoding: 'utf8' }));
            builtHTML = userTemplate(templateInfo);
        }
        ...
        util.printLog('info', `${pathName} rendered [${new Date().getTime() - renderStart} ms]`);
        return path;
    });
